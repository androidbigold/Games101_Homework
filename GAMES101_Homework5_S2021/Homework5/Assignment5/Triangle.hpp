                          ...
                          const Vector3f& dir, float& tnear, float& u, float& v)
{
    ...
    // Moller Trumbore Algorithm ([u, v] is barycentric coordination)
    // orig + dir * tnear = (1 - u - v) * v0 + u * v1 + v * v2
    Vector3f e1 = v1 - v0;
    Vector3f e2 = v2 - v0;
    Vector3f s = orig - v0;
    ...
    if (
        tnear >= 0 &&  // ray is intersected with the trangle
        u > 0 &&
        v > 0 &&
        u + v <= 1  // intersection is inside the trangle (how to deal with boundary?)
    ) {
        return true;
    }
